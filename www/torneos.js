var Torneos =
{
	init: function()
	{
		var _this = this;

		this.tabs = new Tabs ('tabs_torneos', 'tabs_torneos_container');
		this.tabs.show('listar');

		this.form_agregar_torneo = new Form ("form_agregar_torneo", "php/agregar-torneo.php");
		this.form_agregar_torneo.onSuccess = function (data)
		{
			_this.form_agregar_torneo.setData({ });
			_this.tabla.load();
			_this.tabs.show('listar');
		};

		this.form_editar_torneo = new Form ("form_editar_torneo", "php/editar-torneo.php");
		this.form_editar_torneo.onSuccess = function (data)
		{
			_this.tabla.load();
			_this.tabs.show('listar');
		};

		this.tabla = new Table ('tabla_torneos', 'php/listar-torneos.php');
		this.tabla.onSuccess = function (data)
		{
			/* ********* */
			var options = "<option value=''></option>";

			for (var i = 0; i < data.length; i++)
				options += "<option value='" + data[i].id + "'>" + data[i].nombre + "</option>";

			var list = document.querySelectorAll("select[name='id_torneo']");
			for (var i = 0; i < list.length; i++)
				list[i].innerHTML = options;

			/* ********* */
			var encabezado = [];
			var datos = [];

			encabezado.push('ID');
			encabezado.push('Liga');
			encabezado.push('Nombre de Torneo');
			encabezado.push('Fecha de Inicio');
			encabezado.push('---');

			for (var i = 0; i < data.length; i++)
			{
				var fila = data[i];

				var temp = [];
				temp.push(fila.id);
				temp.push(fila.nombre_liga);
				temp.push(fila.nombre);
				temp.push(fila.finicio);
				temp.push(
					"<a onclick='Torneos.editar("+fila.id+");'>Editar</a>"+
					"<a onclick='Torneos.eliminar("+fila.id+");'>Borrar</a>"
				);

				datos.push(temp);
			}

			return { header: encabezado, data: datos };
		};

		this.tabla.load();
	},

	editar: function (id)
	{
		var _this = this;

		Ajax.get ('php/get-torneo.php?id=' + id, function (resp)
		{
			_this.form_editar_torneo.setData (resp.data);
			_this.tabs.show('editar');
		});
	},

	eliminar: function (id)
	{
		var _this = this;

		Ajax.get ('php/eliminar-torneo.php?id=' + id, function (resp)
		{
			_this.tabla.load();
		});
	}
};
